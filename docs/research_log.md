...
| 视角 | 百分比 | 含义 |
|---|---|---|
| **整体**（含实验 +  Demo） | **≈ 43%** | 论文能投出去才算 100% |
| **纯工程实现**（L1+L2+L3+脚本） | **≈ 56%** | L2 完全没动，是最大的空洞 |
| **L1 单层**（本文核心机制之一） | **≈ 95%** | 代码+测试+语料+P1 跑通；**论文级证据仍需重选评估集** |
| **实验数据**（论文的命门） | **≈ 10%** | P1 pilot 已出（3 配对点），但**得不出统计结论**；P2–P5 未跑 |
...
| 2 | 验证协议（C1–C3 claim 体系 + kill test 判定树） | 7% | 82% | 5.7% | `evaluation_protocol_v2.md` 已把主线从"更省 token"修正为"有界性 + 不高于基线的预算下质量 + 遗忘保护"；**C2 预算口径已于 P1 前注册为"不高于基线"（§4 #19b）**；判定树未执行 |
| 3 | **L1 事件记忆**（复盘 agent / 事件库 / 遗忘保护 / 注入） | 20% | 95% | 19.0% | 代码+测试齐；语料 6 条唯一 EVENT；**P1 已跑通（6/6 run ok、3/3 配对）**；差论文级评估集 |
| 4 | **L3 季度蒸馏** | 12% | 5% | 0.6% | 只有 `distill_queue_path` 与 `get_rules_context()` 占位（返回 ""） |
| 5 | **L2 情境检索** | 15% | 0% | 0.0% | 未开始。**P1 给出了它的动机**（见 §5 N3 结果 #5） |
| 6 | 实验基础设施（护栏 / 扫描 / 记账 / 结算 / 冻结） | 8% | 88% | 7.0% | 护栏 + 冻结 + 缓存优先取价全部实测有效；P1 end-to-end 打通；**差 `injected_chars` 每-run 记录（见 N3 #6）** |
| 7 | **实验数据**（P1–P5 消融） | 15% | 10% | 1.5% | P1 pilot 3 配对点（无统计意义）；P2–P5 未跑 |
...
```
680 passed, 1 skipped, 0 failed        （排除慢速 tests/test_structured_agents.py）
```
...
| 5 | ✅ **不再是瓶颈（P1 实测推翻旧估价）** | ~~kill test 是 120 runs（20 配对 × 3 轮 × 2 臂）；按 24 min/run = 48 小时墙钟~~ → **P1 实测单次 run 仅 154–178s（≈2.7 min，思考模式已关）**，120 runs ≈ **5.5 h** | 规模可以按原设计跑，无需砍配对或上并发。**仍待决策的是评估集如何盲选**（§2.2.1），不是跑不跑得动 |
...
| 19 | ✅ **已修（选 B：等预算替换）** | **两臂上下文量不对等，与 C2 口径冲突**。原 `_build_past_context` = rules + **L1（仅 on）** + legacy（两臂都有）→ 加法。实测 `off` 8.3k/13.3k/8.6k 字符 → `on` 15.4k/20.4k/15.1k，**平均多 69%（最高 +86%）**；6 条 L1 事件全部与 legacy 日志 (ticker,date) 重复 → `on` 臂胜出无法区分"选得更好"还是"给得更多"，而 C2 正是主攻 claim | 改为**替换**：`event_memory_enabled=False` → legacy（上游基线原样）；`=True` → L3 准则 + L1 事件，**替换** legacy 段。L1(+L3) 两者皆空时用 legacy 兜底（避免某臂完全无记忆；只会稀释效应，不会放大）。**实测（真实语料）**：`off` 8296/13250/8596 → `on` 7119/7119/6495，`on` 不再含 legacy 文本、含 L1 段 ✓ |
| 19b | ✅ **已定稿（选方案 a，预先注册）** | (B) 之后预算**反向不等**：`on` 只有 `off` 的 **69%**（少 31%），因为 L1 是"压缩摘要"、legacy 是"完整决策+反思"。这**不等于** C2 原话的"固定注入 token 预算下" | **在 P1 运行之前**把 C2 口径改为"**注入预算不高于基线**"（用更少上下文达到不劣，是更难满足的主张）。已在 `evaluation_protocol_v2.md` §0.2 写下带日期的注册说明（含"依据是与结果无关的上下文量测量、P1 未运行、方向是收紧而非放宽"三条论证），并同步 §0.3 问答口径、kill test 判定树、`killtest_protocol.md` §5/§8。**报告要求**：论文必须给出两臂实测注入字符/token 及比值（数据已存在 `full_states_log`） |
...
| 21 | ✅ **已修** | **结算可能在持有窗未走完时触发，把短窗收益当成 5 日结果记录**。`_resolve_pending_entries` 只要求"价格数据可用"，**没有任何最短时间检查**；`_fetch_returns` 把 `actual_days` 夹到 `min(holding_days, len-1)`。实证：legacy 日志里有 `[2026-02-11 \| 600519.SS \| Hold \| -1.3% \| -0.1% \| **2d**]` —— 确实发生过 2 日结算。本例 alpha 仅 −0.1% 所以没写事件，但**若短窗内恰好大幅波动，就会写出一条"用 2 日涨跌算出的 5 日 alpha"的 EVENT** | 新增 `settlement_holding_days`（默认 5，可 env 覆盖）；结算时若 `days < holding_days` 则**跳过并留待下次**（沿用"价格数据不足"的既有语义），并打 info 日志。新增 4 项测试（短窗延后 / 完整窗照常 / 可配置 / env 覆盖） |
...
### N2 · **P1 门禁：两个决策** ✅ **均已解决（2026-09-13）**

1. **臂设计口径（§4 #19）** → **选 (B) 等预算替换**：`off` = legacy；`on` = L3 准则 + L1 事件（替换 legacy），L1 为空时 legacy 兜底。已实现并实测。
   - 副产物 **#19b**：`on` 预算反向变成 `off` 的 69%。**论文口径待定**（推荐 (a) 预先改称"等预算或更低预算"），P1 会测出真实 token 数供定稿。
2. **短窗结算（§4 #21）** → **已修**：新增 `settlement_holding_days`，窗口未走完则延后结算。

### N3 · 跑 P1 ✅ **已完成（2026-09-13）**

```powershell
& $PY scripts/run_experiment.py --scenarios exp_scenarios.json --reps 1 --progress
```

**执行层面：6/6 run 全部 `ok`，3/3 配对完成，无 error。单次 run 154–178s** 

| 指标 | on | off |
|---|---|---|
| hit | 0.67 (2/3) | 0.50 (1/2) |
| hold% | 0% | 33% |
| mean tokens | 149,696 | 170,269（on = 88%） |
| mean $ | 0.0437 | 0.0491（on = 89%） |
| **注入字符（确定性）** | **20,733** | **30,142（on = 69%）** |

逐场景评级：`300750.SZ` 两臂都 Underweight（alpha −4.69%，**都对**）；`600519.SS` 两臂都 Underweight（alpha **+12.62%**，**都错**）；`601318.SS` on=Overweight（**对**，+10.47%）／off=**Hold（不计分）**。

**五条必须写进论文解读的结论**：

1. **hit 差 = 1 个观测 —— 而且配对口径下 Δhit = 0。** 0.67 vs 0.50 全部来自 `601318.SS` 那一条（on 给了方向性判断且正确，off 给 Hold 不计分）。**配对比较里只有 2 对两臂都非 Hold**（300750.SZ 都对、600519.SS 都错）→ **配对 Δhit = 0/2 = 0**。也就是说 0.67 vs 0.50 是 **Hold 造成的分母假象**，不是两臂命中率差异。**任何结论都不能下**，且**论文必须显式规定 Hold 的处理口径**（排除该配对／记为第三类／单独报告），否则会出现这种假差。
2. **注入预算差是确定性的、唯一干净的指标**：`on` = `off` 的 **69%**（8,296/13,250/8,596 → 7,119/7,119/6,495）。**这正是 C2 预先注册的口径**，可直接入表。
3. **总 token/成本差是真的，但不能归因于注入量。** 注入差只有 1.2k–6.1k 字符（≈0.9k–4k token），而总 token 差是 19k–34k —— **差一个数量级**，主要来自下游生成长度与工具调用次数的 run-to-run 方差（评级不同会级联）。**论文必须分开报**：注入预算（确定性）≠ 总运行成本（噪声大）。
4. **`mean_alpha` 在两臂完全相同（0.0614），是误导性输出。** 它是**场景的已实现 alpha**（按构造与臂无关），不是组合收益。harness 把它按臂打印，容易被误当成"两臂表现相同"写进论文。**必须改标签或从 per-arm 表里移除。**
5. **最有价值的发现：最相关的那条教训在 store 里，却不在任何一个臂的注入内容里。** 语料里有一条正好警告本次错误（"对高盈利质量、低杠杆、正现金流的龙头，**不能仅凭均线空头排列给 Underweight**"）——实测：
   - 它在 **legacy 决策日志的 `DECISION` 正文**中（L1 事件里没有）；
   - 它属于**另一个标的**（601318.SS），跨标的段只注入 tag + 2–4 句反思，**不含正文**；
   - 结果 `on` 与 `off` **都没看到它**（两臂都给了 Underweight）。
   → 两臂同时失手**不是记忆内容差异**，而是**检索覆盖不足**。**这正是 L2（按情境相似度检索、而非按 ticker 截断）要解决的问题** —— P1 顺手给出了 L2 的实证动机。

6. **harness 缺陷（✅ 已修）**：两臂共用 `full_states_log_<date>.json`，**后跑的臂覆盖先跑的**（实测存档里全是 `off` 臂的 context：8296/13250/8596，正好等于 off 的实测值）。所以"从存档统计两臂实测预算"只对一半有效；on 的值目前靠确定性重算。**已修**：`run_experiment.py` 新增 `injected_chars()`，每次 run 结束后立刻读自己的存档并把字符数写进 `p1_runs.csv` 一列 → C2 的预算是**每 run 实测**而非推导。新增 `tests/test_experiment_reporting.py`（12 项）。
   - `injected_chars` 目前对**已跑完的 P1** 是空的（旧 CSV 无该列）——如需回填，直接用确定性重算值 7,119 / 7,119 / 6,495（on）与 8,296 / 13,250 / 8,596（off）。

7. **报告口径缺陷（✅ 已修）**：per-arm 表原本打印 `mean_alpha`，而它是**场景的已实现 alpha**（按构造两臂相同，实测都是 0.0614），按臂打印极易被误读成"两臂表现相同"。
   - **已修**：从 per-arm 表**移除**该列，改为表下单独一行并标注 `arm-independent, NOT an arm-level return`；新增 `inj_chars` 列（C2 要报的量）、`scored observations / Hold (unscored)` 行、`scored pairs (both arms non-Hold)` 行、以及**样本量 <10 时的显著性警告**。
   - **副产物**：新报表立刻暴露了上面 #1 的事实——**配对可计分对只有 2，Δhit = 0**。这条以前被 `0.67 vs 0.50` 掩盖了。

8. **速度发现（推翻旧估价）**：单次 run **154–178s ≈ 2.7 min**，因为管线思考模式已关闭。**kill test 120 runs ≈ 5.5 h，而不是按 626s/run 估的 48 h。** → `docs/research_log.md` §4 #5 与 §7 第 1 条（"要问老师 kill test 排不下"）需要重估：**规模已不是瓶颈**，可以按原设计 20 配对 × 3 轮追求有统计功效的规模；仍要问老师的是"**评估集怎么盲选**"，而不是"跑不跑得动"。
...
| 2026-09-11 | 放弃"省 token"作为卖点 | ACON/SimpleMem 是真压缩且量纲不同，比不过；改为 C1 有界性 + C2 不高于基线的预算下质量 + C3 遗忘保护 |
...
| 2026-09-12 | 保留 legacy 决策日志注入在**两臂都有** | 保证两臂唯一差异就是 L1，比较才干净。**⚠️ 此决定于 2026-09-13 被推翻**，见本表最后一行 |
...
| 2026-09-13 | **回退多信号准入**（`pnl`/`regime`/`risk`），L1 保持"只按 \|alpha\| 阈值准入" | 决定**不再对记忆侧做功能改动**。功能本身默认关闭、不影响行为，但被一并撤掉以减少代码面。`scripts/audit_triggers.py` 同步删除。备份已按要求删除（不可恢复）。**保留**：时间切片、`as_of` 必填、`_readonly` 守卫、破坏性写入修复、思考模式修复——那些是安全修复不是功能 |
| 2026-09-13 | **C2 口径预先注册为"注入预算不高于基线"**（原为"固定/同等预算"） | 两臂实现为等预算替换，而 L1 是压缩摘要 → 实测 `on` 仅为 `off` 的 69%。修正在 **P1 运行之前**、依据是**与结果无关**的上下文量测量，且方向是**收紧**（从"同等预算不劣"变成"更低预算不劣"）。另一条路（调高 `on` 预算去对齐）才是看着数据调参，已排除 |
| 2026-09-13 | **两臂改为等预算替换**（`off`=legacy；`on`=L3+L1，替换 legacy；L1 为空时 legacy 兜底） | 原加法设计让 `on` 臂多 54%~86% 上下文，胜出无法区分"选得更好"与"给得更多" |
| 2026-09-13 | 取价**缓存优先**，裸 yfinance 仅兜底 | 不是记忆语义改动（阈值/准入/遗忘/注入/条目格式全不动，只换价格来源）。理由：①限流会让**评分和结算同时静默失效**；②减少实时请求量。**注意缓存文件按"当天"命名、每天滚动，不是永久档案** —— 论文数字产出时须把 `data_cache_dir` 复制成冻结快照 |
| 2026-09-13 | 结算要求**持有窗走完**才落账（`settlement_holding_days`） | 原先只要求"有价格数据"，导致同日/次日重跑会把 1–2 日收益记成 5 日结果（有 `2d` 实证） |
| 2026-09-13 | **P1 结果不作为证据**，且 per-arm 表里的 `mean_alpha` 不得当表现指标 | P1 的 hit 差（0.67 vs 0.50）全部来自 **1 个观测**；`mean_alpha` 是**场景已实现 alpha**、按构造与臂无关（两臂都是 0.0614），harness 却按臂打印，误导性极强。**已改报表**：移除 `mean_alpha` 列、新增 `inj_chars` / `scored observations` / `scored pairs` / 样本量警告 |
| 2026-09-13 | **Hold 必须显式定口径**（排除该配对 / 记为第三类 / 单独报告） | 新报表立刻暴露：配对可计分对只有 2，**配对 Δhit = 0**；`0.67 vs 0.50` 是 Hold 造成的分母假象。不定口径就会出现假差 |
| 2026-09-13 | A/B 的注入预算**每 run 实测**（`injected_chars` 列），不再从存档反推 | 两臂共用 `full_states_log_<date>.json`，后跑的臂覆盖先跑的 → 存档只剩一个臂。C2 的证据应当是实测而非推导 |

---

## 7. 待问老师的问题 → 已迁移到 `docs/questions_for_advisor.md`

**本节的清单已全部迁入 `docs/questions_for_advisor.md`（第 1 轮，13 题，含背景/选项/我的倾向 + `**答：**` 待填行）。**
以后统一在那个文件里回答；我读取后会据此更新本文件、`experiment_plan.md`、`evaluation_protocol_v2.md`。
**此处不再维护第二份清单，避免两处不一致。**

该文件当前 13 题分组为：A 实验设计口径（评估集盲选 / Hold 处理 / kill test 规模 / 降配档）、
B 声明与限制（`get_fundamentals` 回溯泄漏 / A 股数据源 / alpha 做 regime 代理 / 工具调用可审计）、
C 机制与后续（淘汰优先级 / 复盘档位 / L2 主线）、D 产出与投稿（Demo 数据共用 / 投稿目标与 deadline）。
